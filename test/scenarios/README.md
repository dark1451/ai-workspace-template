...
> **사용자 프로젝트의 `pnpm work:<role>` 과 같은 코드·같은 프롬프트를 공유합니다.**
> 템플릿 저장소(이 저장소)에서는 `pnpm test:scenario:<role>` 가 sandbox 안의
> `scripts/agent-runner.mjs` 를 동적 import 해서 호출하고, 사용자 프로젝트에서는
> `scripts/work.mjs` 가 같은 함수를 호출합니다. 첫 메시지 텍스트도 양쪽 모두
> `.cursor/role-prompts/<role>.md` 본문에서 읽어 옵니다. 따라서 프롬프트를 바꾸려면
> 그 파일 하나만 편집하면 work 와 test:scenario 양쪽에 반영됩니다.
...
6. **공통 코어**: sandbox 안에 `scripts/agent-runner.mjs` 가 있으면 (= 새 sandbox) 그 모듈을 동적 import 해 사용자 프로젝트의 `pnpm work:<role>` 과 동일한 코드 경로로 실행합니다. 옛 sandbox 라면 내장 fallback 으로 동작하며, 통합 경로로 가려면 `--clean` 으로 한 번 재생성하면 됩니다.
