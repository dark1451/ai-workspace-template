...
- **모두 통과**: 동일 역할(테스트) 에이전트에게 동료 리뷰 요청.
  ...
  - 리뷰 통과 시: 작업 진행 모드가 **`approval_first`**이면 `done` 전에 사용자에게 검증 요약을 보여 주고 **"이대로 완료할까요?"** 를 받는다. 답 전에는 `waiting_user`로 두거나, 사용자가 이미 "만들어 주세요" 등으로 범위를 승인한 턴이 있으면 `done`으로 전이하고 `testResult`에 승인 근거를 한 줄 남긴다.
  - 리뷰 통과 시 (일반): status를 `done`으로 변경, `board.json` 갱신.
  - 리뷰 수정 요청 시: 검증 절차 재확인 후 재리뷰.
- **실패**: status를 `in_progress`로 되돌리고, 실패 사유를 태스크 메모에 상세히 기록. `board.json` 갱신.

## in_test 가 비어 있을 때

- 회귀 테스트 보강, E2E 시나리오 추가, 커버리지가 약한 영역 식별 등 fallback 후보를 1~3가지 정리해 안내한다.
- 위 후보도 마땅한 게 없으면 공통 규칙(`project-defaults` 의 "할 일 없음 보고 흐름") 4단계로 마무리한다. 테스트 에이전트는 `docs/next-actions.md` 누적 기록을 하지 않고 메시지로만 안내한다 (PM 만 기록).
