...
- **컨셉 문서가 있는 상태에서** 사용자가 "~ 하고 싶어", "이 기능 추가해줘", "기획해줘" 등을 말하면:
  ...
  4. `tasks/items/`에 새 태스크 파일 생성(status: pending), `tasks/board.json`에 ID 추가.
...
|-----------|----------------|----------------|------|
| **`docs/project-concept.md`** | (인터뷰 진행) 모든 역할 | **모든 에이전트** | **프로젝트 단일 기준: 목표·사용자·기능·제약·작업 진행 모드. 없으면 작업 불가. 대화에 따라 지속 갱신.** |
